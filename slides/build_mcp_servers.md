...
- [~] Specification: document + JSON schema; latest version: [`2025-06-18`](https://modelcontextprotocol.io/specification/2025-03-26) (first version `2024-11-05`)
...
- [~] Not backed by a foundation, driven by Anthropic
- [~] ~~No clear development/review process~~
  - [~] [Contributing to Model Context Protocol](https://github.com/modelcontextprotocol/modelcontextprotocol/blob/main/CONTRIBUTING.md)
  - [~] [Contributor Communication](https://modelcontextprotocol.io/community/communication) (July 2025)
- [~] No TCK (Technology Compatibility Kit)
  - [~] But it's one of the [priority areas](https://modelcontextprotocol.io/development/roadmap#validation) for next releases!
...
- [~] Is "Streamable HTTP" better than the legacy "HTTP/SSE"? I'm not so sure...

---

### Part 2 - Quarkus MCP server - history, goals & design
...
- [~] Programmatic API: `@Inject` a manager bean and use a fluent API to add/remove a feature
...
Demo time! Let's use the Dev UI, the MCP Inspector and Goose!
...
- [~] Execution model is determined by the method signature (e.g. `Uni` return type) and additional annotations such as `@Blocking`, `@NonBlocking`, `@RunOnVirtualThread` or `@Transactional`
...
### Part 4 - Advanced features & developer joy
(a small part)
...
### Tools - Hibernate Validator Integration <span class="demo">👀</span>

```java[1: 1-20|11-13|15-16]
import io.smallrye.mutiny.Uni;
...
public class Tools {
    ...
    @Tool(description = """
            ...
            """)
    Uni<List<Monster>> listMonsters(
            @ToolArg(description = "Minimal number of hit points", defaultValue = "1") 
            @PositiveOrZero int minimalHitPoints) {
        return Monster.list("where hitPoints >= :minimalHitPoints",
